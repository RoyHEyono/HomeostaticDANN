#!/usr/bin/env bash
#SBATCH --array=0-7%2              # 8 grid configurations: 0..7
...
#SBATCH --job-name=homeostasis_feedback_alignment
...
# >>> Only 'full' string label <<<
string_labels=("fa_center")
...
if (( num_brightness_factors * num_normtype_detach * num_string_labels != 8 )); then
    echo "WARNING: Grid size != 8; update --array accordingly." >&2
fi
