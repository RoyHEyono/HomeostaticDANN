...
brightness_factor=$(echo $random_params | python -c "import sys, json; config=json.load(sys.stdin); print(config['brightness_factor'])")
brightness_factor_val=$(echo $random_params | python -c "import sys, json; config=json.load(sys.stdin); print(config['brightness_factor_eval'])")
...
python /home/mila/r/roy.eyono/HomeostaticDANN/models/dense_mnist_task/src/train.py \
  ...
  --data.brightness_factor_eval=$brightness_factor_val \
  ...
  --exp.wandb_project=Luminosity_Brightness_Robustness \
